...
var create_img = "hospital_floor";
var create_type = "background";
var zindex = 0; //максимальный z-index
var keys_pressed = [];
var objects = [];
var pix = 3;
var step = 10;
var selected_object = 0; //последний выделенный объект
download();
var Obj = /** @class */ (function () {
    function Obj(folder, image) {
        this.X = 0; //ось Х
        this.Y = 0; //ось У
        this.Z = 0; //ось Z
        this.bottom = 0; //основание
        this.max_frame = 0; //максимальный кадр
        this.min_frame = 0; //минимальный кадр
        this.current_frame = 0; //текущий кадр
        this.delay = 0; //задержка в кадрах
        this.current_delay = 0; //текущая задержка
        this.select = 0; //выделение объекта
        this.Height = 0; //высота по фото
        this.Width = 0; //ширина физическая (отдельно указывается и учитывается вместо ширины фото при столкновении объектов)
        this.actions = []; //список доступных действий
        this.type = folder;
        this.img = image;
        ...
        if (folder == "mobile_objects") {
            if (image == "protagonist") {
                this.delay = 2;
        ...
        if (folder == "static_objects") {
            this.min_frame = 0;
            this.max_frame = 0;
            if (image == "hospital_bed") {
                this.bottom = 15;
                this.Width = 50;
            }
            if (image == "operation_table") {
                this.bottom = 10;
                this.Width = 52;
            }
            if (image == "house_flower") {
                this.bottom = 7;
                this.Width = 17;
            }
            if (image == "boiler_room_lamp") {
                this.delay = 1;
                this.min_frame = 0;
                this.max_frame = 1;
            }
            if (image == "hospital_window") {
                this.noise = { audio: "rain", rad: 200 };
                this.delay = 1;
                this.min_frame = 0;
                this.max_frame = 2;
            }
            if (image == "hospital_wall") {
                this.bottom = 20;
                this.Width = 100;
            }
            if (image == "hospital_door_inside_top") {
                this.bottom = 100;
                this.Width = 20;
            }
            if (image == "hospital_door_inside_bot") {
                this.bottom = 75;
                this.Width = 20;
            }
            if (image == "hospital_wall_inside") {
                this.bottom = 100;
                this.Width = 20;
            }
            if (image == "blood_grinder") {
                this.bottom = 16;
                this.Width = 100;
            }
            if (image == "brick_wall") {
            ...
            if (image == "blood_lights") {
                this.delay = 10;
                this.min_frame = 0;
                this.max_frame = 3;
            }
            if (image == "dead_worker") {
                this.bottom = 5;
                this.Width = 25;
            }
            if (image == "mourner") {
...
function key_down(e) {
    ...
    if (evtobj.keyCode == 46 || evtobj.keyCode == 27)
        select(0);
    if (evtobj.keyCode == 69)
        add_obj(evtobj.keyCode);
    if (evtobj.keyCode == 107)
        pix++;
    if (evtobj.keyCode == 109)
        if (pix != 1)
            pix--;
...
function key_pressed(keycode) {
    for (var obj = 0; obj < objects.length; obj++) {
        if (objects[obj].select == 1 && objects[obj].img != "space") {
            if (keycode == 82)
                objects[obj].Z = objects[obj].Z + step;
            if (keycode == 70)
                objects[obj].Z = objects[obj].Z - step;
            if (keycode == 68)
                objects[obj].X = objects[obj].X + step * move(obj, "RIGHT");
            if (keycode == 65)
                objects[obj].X = objects[obj].X - step * move(obj, "LEFT");
            if (keycode == 87)
                objects[obj].Y = objects[obj].Y - step * move(obj, "UP");
            if (keycode == 83)
                objects[obj].Y = objects[obj].Y + step * move(obj, "DOWN");
        }
    }
}
function move(obj_number, direct) {
    current_obj = step;
    for (var obj = 0; obj < objects.length; obj++) {
        if (objects[obj].Width != 0 && objects[obj_number].Width != 0 && obj_number != obj && objects[obj].img != "space") {
            if (Math.abs(Math.abs((objects[obj_number].X - objects[obj_number].Width / 2) - (objects[obj].X + objects[obj].Width / 2)) - Math.abs((objects[obj_number].X + objects[obj_number].Width / 2) - (objects[obj].X - objects[obj].Width / 2))) < (objects[obj].Width + objects[obj_number].Width)) {
                if (direct == "DOWN" && objects[obj_number].Y - objects[obj_number].bottom < objects[obj].Y - objects[obj].bottom && objects[obj_number].Y + step >= objects[obj].Y - objects[obj].bottom && (objects[obj].Y - objects[obj].bottom - objects[obj_number].Y) < current_obj)
                    current_obj = objects[obj].Y - objects[obj].bottom - objects[obj_number].Y;
                if (direct == "UP" && objects[obj_number].Y - objects[obj_number].bottom > objects[obj].Y - objects[obj].bottom && objects[obj_number].Y - objects[obj_number].bottom - step <= objects[obj].Y && (objects[obj_number].Y - objects[obj_number].bottom - objects[obj].Y) < current_obj)
                    current_obj = objects[obj_number].Y - objects[obj_number].bottom - objects[obj].Y;
            }
            if (Math.abs(Math.abs((objects[obj_number].Y) - (objects[obj].Y - objects[obj].bottom)) - Math.abs((objects[obj].Y) - (objects[obj_number].Y - objects[obj_number].bottom))) < (objects[obj].bottom + objects[obj_number].bottom)) {
                if (direct == "LEFT" && objects[obj_number].X > objects[obj].X && (objects[obj_number].X - objects[obj_number].Width / 2) - step < (objects[obj].X + objects[obj].Width / 2) && ((objects[obj_number].X - objects[obj_number].Width / 2) - (objects[obj].X + objects[obj].Width / 2)) < current_obj)
                    current_obj = ((objects[obj_number].X - objects[obj_number].Width / 2) - (objects[obj].X + objects[obj].Width / 2));
                if (direct == "RIGHT" && (objects[obj_number].X) < (objects[obj].X) && (objects[obj_number].X + objects[obj_number].Width / 2 + step) > (objects[obj].X - objects[obj].Width / 2) && ((objects[obj].X - objects[obj].Width / 2) - (objects[obj_number].X + objects[obj_number].Width / 2)) < current_obj)
                    current_obj = ((objects[obj].X - objects[obj].Width / 2) - (objects[obj_number].X + objects[obj_number].Width / 2));
            }
        }
    }
    if (current_obj == step) {
        if (direct == "LEFT") {
            if (objects[obj_number].Width == 0 && (objects[obj_number].X - (objects[obj_number].realWidth / 2) - step) < 0)
                current_obj = objects[obj_number].X - objects[obj_number].realWidth / 2;
            if (objects[obj_number].Width != 0 && (objects[obj_number].X - (objects[obj_number].Width / 2) - step) < 0)
                current_obj = objects[obj_number].X - objects[obj_number].Width / 2;
        }
        else if (direct == "UP") {
            if (objects[obj_number].bottom != 0 && objects[obj_number].Y - objects[obj_number].bottom - step < 0)
                current_obj = objects[obj_number].Y - objects[obj_number].bottom;
            if (objects[obj_number].bottom == 0 && objects[obj_number].Y - objects[obj_number].Height - step < 0)
                current_obj = objects[obj_number].Y - objects[obj_number].Height;
        }
    }
...
function save() {
    for (var i = 0; i >= 0; i++) {
        if (localStorage["num_" + i] != undefined)
            delete localStorage["num_" + i];
        else
            break;
    }
    for (var obj = 0; obj < objects.length; obj++) {
        if (objects[obj] != undefined) {
            if (objects[obj].img == "space")
                localStorage.setItem("num_" + obj, "space");
            else {
...
function render(obj, obj_number) {
    if (objects[obj_number].img != "space") {
        ...
        if (objects[obj_number].type == "background")
            current_obj.style.zIndex = objects[obj_number].Y;
        if (objects[obj_number].type == "static_objects" || objects[obj_number].type == "mobile_objects")
            current_obj.style.zIndex = zindex + objects[obj_number].Y;
        if (objects[obj_number].type == "shadows")
            current_obj.style.zIndex = zindex * 2 + objects[obj_number].Y;
        ...
        if (objects[obj_number].type == "background")
            current_obj.style.zIndex = zindex * 4 + objects[obj_number].Y;
        if (objects[obj_number].type == "static_objects" || objects[obj_number].type == "mobile_objects")
            current_obj.style.zIndex = zindex * 5 + objects[obj_number].Y;
        if (objects[obj_number].type == "shadows")
            current_obj.style.zIndex = zindex * 3 + objects[obj_number].Y;
        if (objects[obj_number].select == 0)
            current_obj.style.zIndex = current_obj.style.zIndex + zindex * 6;
...
function select(obj_number) {
    if (keys_pressed[17] != 1) {
        for (var obj = 0; obj < objects.length; obj++) {
            if (keys_pressed[46] == 1 && objects[obj].select == 1) {
    ...
    if (keys_pressed[27] == 1)
        objects[obj_number].select = 0; //если нажат ESC снять выделение
    else if (keys_pressed[46] != 1) {
        if (keys_pressed[17] == 1 && objects[obj_number].select == 1)
            objects[obj_number].select = 0;
        else
            objects[obj_number].select = 1;
