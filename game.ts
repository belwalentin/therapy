...
let map_width:number = 320;		//ширина сцены
let map_height:number = 180;		//высота сцены
let map_top:number = 0;		//координаты сцены по игровой системе координат (в итоге сцена будет выравняна по середине экрана)
let map_left:number = 0;
let pix: number = 3;
let pause:boolean = true;
let scenario:string = "awakening";
let current_volume: number[]=[];
let switching: string = "ward";
let current_map: string = "ward";
...
let gap: boolean = true; 	//проверяет наличие пустых зон при смещении сцены
let dialogues: any[] = [];
...
let uses:[number, number, number];
uses = [3,69, 0];  //0 - нет  1 - потенц 2 -  анимация 3 - статика 4 - анимация сползания
let control:boolean = true;
let step: number =3;
let step_facet: number =0;
let protagonist: number;
let zindex: number =map_height;
...
if(current_obj)current_obj.parentNode.removeChild(current_obj);
...
for(let obj:number=0; obj<objects.length; obj++){
		objects[obj]=JSON.parse(objects[obj]);
		if(objects[obj].img=="protagonist")protagonist=obj;
		if(objects[obj].img=="space")alert("НАЙДЕТ ПУСТОЙ ОБЪЕКТ ТВОЯ СИСТЕМА НЕ РАБОТАЕТ "  );
}
...
function plot(obj:any){
	if(scenario=="awakening"){
			if(dialog[0]==0)scenario="acquaintance";
	}
...
if(objects[num].img=="protagonist" && (dialog[0]==0 || (dialog[0]!=0 && uses[0]==5)) && control==true){	
	...
	if(keycode==68)objects[num].X=objects[num].X+step*move(objects[num], "RIGHT");
	if(keycode==65)objects[num].X=objects[num].X-step*move(objects[num], "LEFT");
	if(keycode==87)objects[num].Y=objects[num].Y-step*move(objects[num], "UP");
	if(keycode==83)objects[num].Y=objects[num].Y+step*move(objects[num], "DOWN");

if(keys_pressed[87]==1 || keys_pressed[83]==1)step_facet=map_width;
if(keys_pressed[68]==1 || keys_pressed[65]==1)step_facet=map_height;

for(let gor: number = 0; gor<step;  gor++){	
	for(let ver: number = 0; ver<step_facet;  ver++){			
		for(let obj_num: number=0; obj_num<objects.length; obj_num++){
...
if(keys_pressed[68]==1  && map_left<=objects[protagonist].X-map_width/2)map_left++;
if(keys_pressed[65]==1  && map_left>=objects[protagonist].X-map_width/2)map_left--;
if(keys_pressed[87]==1 && map_top>=objects[protagonist].Y-(objects[protagonist].bottom*2)-map_height/2)map_top--;
if(keys_pressed[83]==1  && map_top<=objects[protagonist].Y-(objects[protagonist].bottom*2)-map_height/2)map_top++;	
}
}
}
...
if(uses[0]<=1){
	...
	for(var current_noise in current_volume){
		...
		if(current_volume[current_noise]<=1)noise.volume=Math.round(current_volume[current_noise]*10)/10;
		else noise.volume=1;
...
if(dialog[0]==0){
    ...
    for (let obj:number=0; obj<objects.length; obj++) {	
	  ...
	  if(( (objects[obj].X+objects[obj].realWidth/2)<map_left-step*5) || ( (objects[obj].X-objects[obj].realWidth/2)>map_left+map_width+step*5) || (  (objects[obj].Y-objects[obj].Height-objects[obj].Z)>map_top+map_height+step*5) || (  (objects[obj].Y-objects[obj].Z)<map_top-step*5)){
			...
			current_obj.parentNode.removeChild(current_obj);
     	     ...
     	     else{		//если объект должен быть в кадре
     	   if(current_obj){
	...
	if((objects[obj].current_delay==0 && (objects[obj].max_frame-objects[obj].min_frame>0))||(objects[obj].max_frame-objects[obj].min_frame==0 && objects[obj].type=="mobile_objects")){
	 current_obj.src = "" + objects[obj].type + "/" + objects[obj].img + "" + Math.floor((objects[obj].current_frame+objects[obj].min_frame)/10) + "" + (objects[obj].current_frame+objects[obj].min_frame)%10 + ".png";
    	...
    	if(objects[obj].Y>zindex)zindex=objects[obj].Y;

  	if(objects[obj].type=="background")current_obj.style.zIndex = objects[obj].Y;
   	if(objects[obj].type=="static_objects" || objects[obj].type=="mobile_objects")current_obj.style.zIndex = zindex + objects[obj].Y;
      	if(objects[obj].type=="shadows")current_obj.style.zIndex = zindex*2 + objects[obj].Y;  
	if(objects[obj].img=="hospital_wall"){
		if(objects[obj].Y>objects[protagonist].Y){
				current_obj.style.opacity="0";
			if(objects[protagonist].Y>objects[obj].Y-objects[obj].Height) current_obj.style.opacity="0.7";
			}
			else current_obj.style.opacity="1";
		}
     	     	}
     	     	else{
     	     		...
     	     		if(cutscene[0]==1)cutscenes[0][0]=obj;
     	     	}
     	     }
...
if(pause == false && cutscene[0]==0){
	...
	if(objects[obj].type != "mobile_objects")animation(objects[obj]);
    ...
    if(objects[obj].img=="protagonist" && dialog[0]!=0 && uses[0]>1)animation(objects[obj]);


if(uses[0]<=1)using(uses[1]);
else if(uses[1]==obj)using(uses[1]);


    }
    else steps.volume=0;
    if(objects[obj].img=="protagonist"){
    	if(dialog[0]!=0) steps.volume=0;
     	else if(uses[0]>1)steps.volume=0;   	
...
if (cutscene[0]==1){
	...
	current_obj.style.zIndex=10000;		
	if(cutscenes[0][1]==5)cutscenes[0][1]=0;
	else cutscenes[0][1]++;
}
...
if(dialog[0]==0){
	...
	if(uses[0]==5 || uses[0]<=1)stop_move(protagonist);
...
function goto_obj(a: number, X, Y, dist:number){
...
function using(obj_num) {
	current_obj=document.getElementById("using");
	if(uses[0]==0)current_obj.innerHTML="";
	else if (uses[0]==1){
		if(objects[obj_num].img=="hospital_bed" || objects[obj_num].img=="operation_table")current_obj.innerHTML="E - Прилечь.";	
	}	
	else if (uses[0]==2){
		control=false;
...
function key_down(e: any) {
 ...
 if(cutscene[0]==0){
	...
	dialog[1]=dialog[1]+2;
 	...
 	if(uses[0]<=1)objects[protagonist].current_delay=objects[protagonist].delay;
...
function key_up(e: any) {
	...
	if(evtobj.keyCode==68 || evtobj.keyCode==65|| evtobj.keyCode==87 || evtobj.keyCode==83){
	    if(keys_pressed[68]>keys_pressed[evtobj.keyCode]) keys_pressed[68]=keys_pressed[68]/2;
	    if(keys_pressed[65]>keys_pressed[evtobj.keyCode]) keys_pressed[65]=keys_pressed[65]/2;
	    if(keys_pressed[87]>keys_pressed[evtobj.keyCode]) keys_pressed[87]=keys_pressed[87]/2;
	    if(keys_pressed[83]>keys_pressed[evtobj.keyCode]) keys_pressed[83]=keys_pressed[83]/2;
...
function move(obj_mov: any, direct: string){
	 	current_obj=step;
		for(let obj:number=0; obj<objects.length; obj++){		
			if( objects[obj].Width!=0&&  obj_mov!=objects[obj] && (objects[obj].type=="static_objects" || objects[obj].type=="mobile_objects")){ 	//ЕСЛИ НЕ РАССМАТРИВАЕТСЯ ВЫБРАНЫЙ ОБЪЕКТ, НЕ РАССМ ПУСТОЙ И РАССМ ЯВЛЯЕТСЯ ОБЪЕКТОМ
				...
				if(  Math.abs(Math.abs((obj_mov.X-obj_mov.Width/2) - (objects[obj].X+objects[obj].Width/2)  ) - Math.abs((obj_mov.X+obj_mov.Width/2) - (objects[obj].X-objects[obj].Width/2)))<(objects[obj].Width+obj_mov.Width)){
					if (direct =="DOWN"  && obj_mov.Y-obj_mov.bottom< objects[obj].Y-objects[obj].bottom  && obj_mov.Y+step>=objects[obj].Y-objects[obj].bottom && (objects[obj].Y-objects[obj].bottom-obj_mov.Y)<current_obj)current_obj=objects[obj].Y-objects[obj].bottom-obj_mov.Y;
					if (direct =="UP"  && obj_mov.Y-obj_mov.bottom>objects[obj].Y-objects[obj].bottom  && obj_mov.Y-obj_mov.bottom-step<=objects[obj].Y && (obj_mov.Y-obj_mov.bottom-objects[obj].Y)<current_obj)current_obj=obj_mov.Y-obj_mov.bottom-objects[obj].Y;
				...
				if(Math.abs(Math.abs((obj_mov.Y)-(objects[obj].Y-objects[obj].bottom)) - Math.abs((objects[obj].Y)-(obj_mov.Y-obj_mov.bottom))) <(objects[obj].bottom+obj_mov.bottom)){
					if (direct =="LEFT"  && obj_mov.X>objects[obj].X && (obj_mov.X-obj_mov.Width/2)-step<(objects[obj].X+objects[obj].Width/2) && ((obj_mov.X-obj_mov.Width/2)-(objects[obj].X+objects[obj].Width/2))<current_obj)current_obj = ((obj_mov.X-obj_mov.Width/2)-(objects[obj].X+objects[obj].Width/2));	
					if (direct =="RIGHT"  && (obj_mov.X)<(objects[obj].X) && (obj_mov.X+obj_mov.Width/2+step)>(objects[obj].X-objects[obj].Width/2) && ((objects[obj].X-objects[obj].Width/2)-(obj_mov.X+obj_mov.Width/2))<current_obj)current_obj = ((objects[obj].X-objects[obj].Width/2)-(obj_mov.X+obj_mov.Width/2));		
				}
...
if(current_obj==step && obj_mov.img=="protagonist"){
	if(direct=="UP" && (obj_mov.Y-obj_mov.bottom-step)<=map_top)current_obj=((obj_mov.Y-obj_mov.bottom)-map_top);
	if(direct=="DOWN" && (obj_mov.Y+step>map_top+map_height))current_obj=map_top+map_height-obj_mov.Y;

	if(direct=="LEFT" && obj_mov.X-obj_mov.Width/2-step<=map_left)current_obj=(obj_mov.X-obj_mov.Width/2)-map_left;

	if(direct=="RIGHT" && obj_mov.X+obj_mov.Width/2+step>=map_left+map_width)current_obj=(map_left+map_width)-(obj_mov.X+obj_mov.Width/2);
